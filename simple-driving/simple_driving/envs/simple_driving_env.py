...
class SimpleDrivingEnv(gymnasium.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        p.setGravity(0, 0, -10)
        # Reload the plane and carl
        Plane(self.client)
        self.car = Car(self.client)

        # Set the goal to a random target
        x = (self.np_random.uniform(5, 9) if self.np_random.integers(1,2) else
             self.np_random.uniform(-5, -9))
        y = (self.np_random.uniform(5, 9) if self.np_random.integers(1,2) else
             self.np_random.uniform(-5, -9))
        self.goal = (x, y)
        self.done = False
